...
print_menu() {
    ...
    log "    $script [${YELLOW}-uh${RESET}] [${YELLOW}-n ${GREEN}\"namespace\"${RESET}]"
    log ""
...
if [[ "$dry_run" -eq 1 ]]; then
    ...
    info "Tests to perform:"
    for c in "${CHARTS[@]}"; do
        if [[ "$c" = "cert-manager" ]]; then
            info "Chart bitnami/${c} was successfully installed into namespace 'kube-system'"
        else
            info "Chart bitnami/${c} was successfully installed into namespace '$namespace'"
        fi
    done
    info ""
    exit 0
...
for c in "${CHARTS[@]}"; do
    if [[ "$c" = "cert-manager" ]]; then
        silence helm status "$c" -n "kube-system" && info "$c successfully installed in namespace 'kube-system'!" || error "$c wasn't installed successfully"
    else
        silence helm status "$c" -n "$namespace" && info "$c successfully installed in namespace '$namespace'!" || error "$c wasn't installed successfully"
    fi
done
